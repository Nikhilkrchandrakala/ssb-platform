...
const EMPTY_FORM_DATA: EnquiryFormData = {
  firstName: "",
  lastName: "",
  email: "",
  phone: "",
...
function parseSplashMessage(msg: string) {
...
// Format YYYY-MM-DD from the native date picker into Zoho's "MMM D, YYYY" format.
function formatDateToZoho(dateStr: string) {
  if (!dateStr) return "";
...
/**
 * Ported from legacy pages/From.jsx. Renders the multi-step SSB enquiry form
 * that posts directly to Zoho CRM's Web-to-Lead endpoint (third-party, kept
 * as-is — not one of our own /api routes). Used on the About, Contact and
 * Hall of Fame pages.
 *
 * Deliberately dropped vs. legacy: the `isModal` embed variant (unused by any
 * ported page), the Zoho WebForm Analytics tracking call and SalesIQ visitor
 * identification (both optional, best-effort analytics wrapped in try/catch
...
export default function EnquiryForm() {
  ...
  // Load Zoho WebForm Analytics script dynamically, matching legacy behaviour.
  useEffect(() => {
  ...
  const handleBlur = (field: string) => {
    ...
    setErrors((prev) => {
      const updated = { ...prev };
      if (validationErrors[field]) updated[field] = validationErrors[field];
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // before FormData reads it, so set Mobile explicitly from state.
    ...
    toast.promise(
      fetch("https://crm.zoho.in/crm/WebToLeadForm", {
        ...
        .then((data: unknown) => {
          ...
          if (typeof data === "object" && data !== null) {
            const obj = data as { actionsubmit?: string; invalidCaptcha?: string; actionvalue?: string };
            if (obj.actionsubmit === "Splash Message") {
              if (obj.invalidCaptcha === "true") {
                throw new Error(obj.actionvalue || "Invalid Captcha");
              }
              successMsg = obj.actionvalue || successMsg;
            }
        ...
        }),
      {
        ...
        error: (err: Error) => `Submission error: ${err.message || "please try again"}`,
      }
    );
  };
    ...
    <section className="enquiry-form-section sectionspace80">
      ...
      <div className="container">
        ...
        >
          ...
          <input type="hidden" name="xnQsjsdp" value="59a079d57b7b3ba45f3e669634efd281e4b7a16ff519bffee62aee08c43304d7" readOnly />
          ...
          <input type="hidden" name="Lead Source" value="Webform" readOnly />

          <input type="hidden" id="ldeskuid" name="ldeskuid" readOnly />
          <input type="hidden" id="LDTuvid" name="LDTuvid" readOnly />
          ...
          <div className="row g-4">
            ...
            {/* Section 3: Are you familiar with us? */}
