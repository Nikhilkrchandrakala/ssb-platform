...
const MANUAL_MODULES = [
  {
    id: "full_course",
    labelBase: "Full 12-day SSB Hackathon",
    defaultPrice: 14999,
  },
  ...MODULES,
];
...
export default function CoursesView() {
  ...
  const [editOfflineSlotId, setEditOfflineSlotId] = useState<string | null>(
    null,
  );
  ...
  const getCoursePrice = (courseId: string, defaultPrice: number) => {
    ...
    return course && typeof course.price === "number"
      ? course.price
      : defaultPrice;
  };
  ...
  useEffect(() => {
    ...
    fetch("/api/allCourses")
      ...
      .then((data: Course[]) => {
        ...
        (data || []).forEach((c) => {
          if (c.courseId && typeof c.price === "number")
            next[c.courseId] = c.price;
        });
        setPriceValues(next);
      })
      .catch((err) => {
  ...
  const totalCapacity = monthBatches.reduce(
    (sum, b) => sum + (b.maxStudents || 0),
    0,
  );
  const totalBooked = monthBatches.reduce(
    (sum, b) => sum + (b.bookedStudents ? b.bookedStudents.length : 0),
    0,
  );
  ...
  const openEditModal = async (id: string) => {
    try {
      ...
      if (slot.startTime)
        setStartDate(toDateInputValue(new Date(slot.startTime)));
  ...
  const openEditOfflineModal = async (id: string) => {
    try {
      ...
      if (slot.startTime)
        setOfflineStartDate(toDateInputValue(new Date(slot.startTime)));
  ...
  const saveOfflineSlot = async () => {
    ...
    const payload = {
      ...
      endTime: new Date(
        new Date(offlineStartDate).getTime() + 86400000,
      ).toISOString(),
    ...
    try {
      ...
      const url = editOfflineSlotId
        ? `/api/updateSlot/${editOfflineSlotId}`
        : "/api/addSlot";
  ...
  const calcManualPrice = () => {
    if (!bookingSlot)
      return { base: 0, discount: 0, netBase: 0, gst: 0, total: 0 };
    let basePrice = 0;
  ...
  const confirmManualBook = async () => {
    ...
    if (!identifier) {
      window.Swal?.fire({
        icon: "warning",
        text: "Please enter a student email or ID.",
        background: "#1a1a1a",
        color: "#fff",
      });
      return;
    ...
    if (
      ...
      window.Swal?.fire({
        icon: "warning",
        text: "Please select at least one course/module.",
        background: "#1a1a1a",
        color: "#fff",
      });
      return;
    ...
    try {
      ...
      const response = await fetch(`/api/manualBookSlot/${bookingSlotId}`, {
        ...
        body: JSON.stringify({
          ...
          selectedModules,
          discountType:
            manualDiscountValue > 0 ? manualDiscountType : undefined,
          discountValue:
            manualDiscountValue > 0 ? manualDiscountValue : undefined,
        }),
      });
  ...
  const saveGlobalPrices = async (e: React.FormEvent) => {
    ...
    PRICING_KEYS.forEach((key) => {
      if (!priceLocked[key])
        updates.push({ key, val: Number(priceValues[key]) });
    });
    ...
    try {
      ...
      for (const update of updates) {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to update price for ${match.title || update.key}`,
          );
        }
      }
  ...
  const openConsolidateModal = (preselectedTargetId?: string) => {
    const target =
      allBatches.find((b) => b._id === preselectedTargetId && !b.isCancelled) ||
      allBatches.find((b) => !b.isCancelled);
    if (!target) {
      window.Swal?.fire({
        icon: "info",
        text: "No active batches available to consolidate.",
        background: "#1a1a1a",
        color: "#fff",
      });
      return;
  ...
  const onTargetBatchChange = (newTargetId: string) => {
    ...
    ).find((b) => b._id === newTargetId);
    if (target) {
      if (target.startTime) {
        setConsolidateNewDate(toDateInputValue(new Date(target.startTime)));
      }
      setConsolidateSourceIds((prev) =>
        prev.filter((id) => id !== newTargetId),
      );
      setConsolidateCapacity(target.maxStudents || 50);
    }
  };
  ...
  const consolidateTargetBatch = (
    consolidateBatches.length > 0 ? consolidateBatches : allBatches
  ).find((b) => b._id === consolidateTargetId);
  const consolidateTargetBooked = consolidateTargetBatch?.bookedStudents
    ? consolidateTargetBatch.bookedStudents.length
    : 0;
  const consolidateSourceBatches = (
    consolidateBatches.length > 0 ? consolidateBatches : allBatches
  ).filter((b) => consolidateSourceIds.includes(b._id));
  const consolidateCombinedTotal =
    consolidateTargetBooked +
    consolidateSourceBatches.reduce(
      (acc, b) => acc + (b.bookedStudents?.length || 0),
      0,
    );
  ...
  const submitConsolidation = async () => {
    if (!consolidateTargetId) {
      window.Swal?.fire({
        icon: "warning",
        text: "Please choose a target batch.",
        background: "#1a1a1a",
        color: "#fff",
      });
      return;
    ...
    if (consolidateSourceIds.length === 0 && !consolidateNewDate) {
      window.Swal?.fire({
        icon: "warning",
        text: "Please select source batches to absorb or choose a revised start date.",
        background: "#1a1a1a",
        color: "#fff",
      });
      return;
  ...
  return (
    <div
      className="container"
      style={{ maxWidth: 1400, margin: "40px auto", padding: "0 20px" }}
    >
      <div className="admin-page-header">
        <div className="header-left">
          <h1 className="admin-page-title">
            <CalendarDays size={20} className="me-2" style={ICON_STYLE} /> Batch
            Management
          </h1>
          <p className="text-muted mb-0">
            Schedule courses, manage batches, and handle manual seat bookings
          </p>
        </div>
        <div className="d-flex gap-2 flex-wrap">
          ...
          <button className="thm-btn" onClick={openAddModal}>
            <PlusCircle size={16} style={ICON_STYLE} /> Create New Online Batch
          </button>
          <button className="thm-btn secondary" onClick={openAddOfflineModal}>
            <Building2 size={16} style={ICON_STYLE} /> Create New Offline Batch
          </button>
        </div>
      </div>
      ...
      <div className="admin-card mb-5" style={{ marginTop: -10 }}>
        ...
        >
          ...
          >
            <Globe size={18} className="me-2" style={ICON_STYLE} /> Global
            Course Module Pricing
          </h4>
          <span
            className="badge"
            style={{
        ...
        <form className="row g-3 align-items-end" onSubmit={saveGlobalPrices}>
            ...
            <div className="col-xl col-md-4 col-sm-6" key={key}>
              ...
              <div className="input-group">
                ...
                <button
                  ...
                  type="button"
                  style={{
                    background: priceLocked[key]
                      ? "var(--surface-light)"
                      : "rgba(224, 194, 20, 0.15)",
          ...
          <div className="col-xl-auto col-md-12 text-end">
            ...
            >
              <Save size={16} style={ICON_STYLE} />{" "}
              {savingPricing ? "Saving..." : "Save Global Prices"}
            </button>
          </div>
        </form>
      </div>
          ...
          <div className="month-stats">
            {loading ? (
              ...
              <>
                ...
                <span className="me-3">CAPACITY: {totalCapacity}</span>{" "}
      ...
      {!loading && monthBatches.length === 0 && (
        ...
        >
          <div
            ...
            style={{ color: "var(--primary-gold)", opacity: 0.3 }}
          >
            <CalendarX2 size={64} />
          </div>
          <h3>No batches scheduled for {`${MONTH_NAMES[month]} ${year}`}</h3>
          <p className="text-muted">
            You haven&apos;t created any course batches for this period yet.
          </p>
        ...
        <div className="row g-4">
          {monthBatches.map((slot) => {
            ...
            if (slot.isFullCourse)
              finalPriceVal = getCoursePrice(
                "full_course",
                slot.price || 12499,
              );
            const priceStr = finalPriceVal
              ? `₹${Number(finalPriceVal).toLocaleString("en-IN")}`
              : "FREE";
              ...
              <div className="col-lg-4 col-md-6" key={slot._id}>
                <div className="batch-card">
                  ...
                  <div className="batch-body">
                    ...
                    <div className="stat-item">
                      <span className="stat-label">
                        <Users size={14} className="me-2" style={ICON_STYLE} />
                        Capacity
                      </span>
                    ...
                    <div className="stat-item">
                      <span className="stat-label">
                        <Tag size={14} className="me-2" style={ICON_STYLE} />
                        Price
                      </span>
                    ...
                    <div className="stat-item">
                      ...
                      <span
                        ...
                        style={{ color: bookingClosed ? "#ff6b6b" : undefined }}
                      >
                        {bookingClosed
                          ? "Closed"
                          : formatTimeRemaining(slot, now)}
                      </span>
                    </div>
                    <div className="mt-3 text-center">
                      {isFull ? (
                        <span
                          className="badge bg-danger w-100 p-2"
                          style={{ borderRadius: 8 }}
                        >
                          <Ban size={14} className="me-2" style={ICON_STYLE} />
                          BATCH FULL
                        </span>
                      ) : (
                        <span
                  ...
                  <div className="batch-footer">
                    <button
                      ...
                      style={{ flex: 1 }}
                      title="Edit"
                      onClick={() => openEditModal(slot._id)}
                    >
                      <Pencil size={14} />
                    </button>
                    <button
      ...
      <div className="admin-page-header" style={{ marginTop: 40 }}>
        <div className="header-left">
          <h1 className="admin-page-title">
            <Building2 size={20} className="me-2" style={ICON_STYLE} /> Offline
            Batches
          </h1>
          <p className="text-muted mb-0">
            In-person batches — students pay a ₹
            {OFFLINE_REGISTRATION_FEE.toLocaleString("en-IN")} registration fee
            online, balance at the center
          </p>
        </div>
      </div>
      ...
      {!loading && offlineBatches.length === 0 && (
        ...
        >
          ...
          <p className="text-muted">
            Create one to start accepting in-person registrations.
          </p>
          <button className="thm-btn mt-3" onClick={openAddOfflineModal}>
            <Plus size={16} style={ICON_STYLE} /> Create New Batch
          </button>
        </div>
      )}
              ...
              <div className="col-lg-4 col-md-6" key={slot._id}>
                <div className="batch-card">
                  ...
                  <div className="batch-body">
                    ...
                    <div className="stat-item">
                      <span className="stat-label">
                        <MapPin size={14} className="me-2" style={ICON_STYLE} />
                        Location
                      </span>
                    ...
                    <div className="stat-item">
                      ...
                      <span className="price-badge">
                        ₹
                        {(
                          slot.price ?? OFFLINE_REGISTRATION_FEE
                  ...
                  <div className="batch-footer">
                    <button
                      ...
                      style={{ flex: 1 }}
                      title="Edit"
                      onClick={() => openEditOfflineModal(slot._id)}
                    >
                      <Pencil size={14} />
                    </button>
                    <button
      ...
      {offlineModalOpen && (
        <div className="admin-modal-overlay" style={{ display: "flex" }}>
          <div
            ...
            style={{ maxWidth: 600, width: "95%", margin: "20px auto" }}
          >
            ...
            <div className="mb-3">
              <label className="admin-form-label">
                <MapPin size={14} className="me-1" style={ICON_STYLE} />{" "}
                Location / Venue*
              </label>
              <input
                type="text"
            ...
            <div className="row mb-3">
              <div className="col-6">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setOfflineMaxStudents(Number(e.target.value))
                  }
                />
              </div>
              <div className="col-6">
                ...
                <small className="text-muted" style={{ fontSize: "0.72rem" }}>
                  Editable for testing — normally kept at ₹
                  {OFFLINE_REGISTRATION_FEE.toLocaleString("en-IN")}.
                </small>
              </div>
            </div>
            ...
            <div className="d-flex justify-content-end gap-2 mt-4 pt-3 border-top border-secondary">
              ...
              <button
                type="button"
                className="thm-btn"
                onClick={saveOfflineSlot}
                disabled={savingOfflineSlot}
              >
                <Save size={14} className="me-1" style={ICON_STYLE} />{" "}
                {savingOfflineSlot ? "Saving..." : "Save Batch"}
              </button>
            </div>
          </div>
        </div>
          ...
          >
            <div className="admin-modal-header">
              <h5 className="admin-modal-title">
                ...
                {editSlotId ? "Edit Batch Details" : "Create New Online Batch"}
              </h5>
              <button
                type="button"
                className="btn-close btn-close-white"
            ...
            <div>
              ...
              <div className="mb-3 form-check" style={{ paddingLeft: "2em" }}>
                ...
                />
                <label
                  className="form-check-label admin-form-label"
                  htmlFor="isFullCourse"
                ...
                >
                  <label className="admin-form-label mb-2">
                    <BookOpen
                      size={14}
                      className="me-1"
                      style={{ ...ICON_STYLE, color: "var(--primary-gold)" }}
                    />{" "}
                  ...
                  {MODULES.map((m) => (
                    <div className="form-check mb-2" key={m.id}>
                      <input
                        ...
                        onChange={(e) =>
                          onModuleCheckChange(m.id, e.target.checked)
                        }
                      />
                      <label
                      ...
                      >
                        ...
                        <span className="text-warning font-weight-bold">
                          &#8377;
                          {getCoursePrice(m.id, m.defaultPrice).toLocaleString(
                            "en-IN",
                          )}
                        </span>
                        )
      ...
      {bookModalOpen && (
        ...
        >
          ...
          >
            <div className="admin-modal-header">
              <h5 className="admin-modal-title">
                <UserPlus size={18} className="me-2" style={ICON_STYLE} />{" "}
                Manual Seat Booking
              </h5>
              <button
                type="button"
            ...
            <div>
              <div className="mb-3">
                ...
                <small className="text-muted" style={{ fontSize: "0.75rem" }}>
                  Enter the registered student&apos;s email or system ID to book
                  manually.
                </small>
              </div>
                  ...
                  {MANUAL_MODULES.map((m) => (
                    ...
                    >
                      <input
                        ...
                        disabled={
                          m.id === "full_course"
                            ? manualAnyOtherChecked
                            : manualChecks.full_course
                        }
                        onChange={(e) =>
                          toggleManualModule(m.id, e.target.checked)
                        }
                      />
                      <label
                        className="form-check-label text-white"
                        style={{ fontSize: "0.85rem" }}
                        htmlFor={`manual_mod_${m.id}`}
                      >
                        {m.labelBase} (
                        <span className="text-warning font-weight-bold">
                          &#8377;
              ...
              >
                <div
                  ...
                  style={{ fontSize: "0.8rem", opacity: 0.8, color: "#fff" }}
                >
              ...
              <div className="modal-footer px-0 pb-0 pt-3">
                ...
                >
                  <CheckCircle2 size={14} className="me-1" style={ICON_STYLE} />{" "}
      ...
      {consolidateModalOpen && (
        <div className="admin-modal-overlay" style={{ display: "flex" }}>
          <div
            className="admin-modal"
            style={{ maxWidth: 680, width: "95%", margin: "20px auto" }}
          >
            <div className="admin-modal-header">
              <h3 className="admin-modal-title">
                <GitMerge size={18} className="me-2" style={ICON_STYLE} />{" "}
                Consolidate &amp; Reschedule Batches
              </h3>
              <button
                type="button"
                className="btn-close btn-close-white"
            ...
            >
              <p className="text-muted small mb-3">
                Cancel source batches, merge their enrolled candidates into a
                destination batch, and optionally shift commencement dates. All
                changes propagate atomically to candidate dashboards with zero
                data loss.
              </p>
              ...
              <div className="mb-3">
                <label
                  className="admin-form-label d-block mb-1"
                ...
                <select
                  ...
                  style={{
                    background: "var(--surface-light)",
                    border: "1px solid rgba(224, 194, 20, 0.4)",
                    borderRadius: 6,
                    color: "#fff",
                  }}
                  ...
                  {(consolidateBatches.length > 0
                    ...
                    .map((b) => (
                      <option key={b._id} value={b._id}>
                        ...
                        {b.mode === "offline" ? "Offline" : "Online"}) ·{" "}
                        {b.startTime
                          ? new Date(b.startTime).toLocaleDateString("en-IN")
                          : "No date"}{" "}
                        ({b.bookedStudents?.length || 0}/{b.maxStudents || 50}{" "}
                        enrolled)
                      </option>
                    ))}
                </select>
              ...
              <div className="mb-3">
                ...
                <small className="text-muted">
                  Update the start date for Batch #
                  {consolidateTargetBatch?.batchNo ||
                    consolidateTargetBatch?.title}
                  . Both existing students and absorbed students will see this
                  date on their dashboard.
                </small>
              </div>
              ...
              <div className="mb-3">
                ...
                >
                  {(consolidateBatches.length > 0
                    ...
                    .filter(
                      (b) =>
                        b._id !== consolidateTargetId &&
                        (b.mode || "online") ===
                          (consolidateTargetBatch?.mode || "online"),
                    )
                    .map((b) => (
                      <label
                        key={b._id}
                        ...
                        style={{
                          background: consolidateSourceIds.includes(b._id)
                            ? "rgba(230, 126, 34, 0.12)"
                            : "rgba(255, 255, 255, 0.03)",
                          border: consolidateSourceIds.includes(b._id)
                  ...
                  {(consolidateBatches.length > 0
                    ...
                    <p className="text-muted small mb-0 p-2">
                      No other compatible batches available to absorb.
                    </p>
                  )}
                </div>
                <small className="text-muted">
                  Checked batches will be cancelled (soft-deleted) and their
                  paid orders transferred to Batch #
                  {consolidateTargetBatch?.batchNo ||
                    consolidateTargetBatch?.title}
                  .
                </small>
              ...
              <div className="mb-3">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConsolidateCapacity(Number(e.target.value))
                  }
                />
                <small className="text-muted">
                  Must be at least {consolidateCombinedTotal} to fit all
              ...
              <div className="form-check mb-4">
                ...
                <label
                  ...
                  htmlFor="consolidateNotifyToggle"
                >
                  Notify all affected candidates on their dashboard with an
                  in-app alert banner
                </label>
              </div>
              ...
              <div
                ...
                style={{
                  background: "rgba(224, 194, 20, 0.08)",
                  border: "1px solid rgba(224, 194, 20, 0.3)",
                }}
              >
                <div
                  className="d-flex justify-content-between mb-1"
                ...
                {consolidateNewDate && (
                  ...
                  >
                    ...
                    <span className="fw-bold text-white">
                      {new Date(consolidateNewDate).toLocaleDateString(
                        "en-IN",
                ...
                >
                  ...
                  <span>
                    {consolidateSourceBatches.length > 0
                      ? consolidateSourceBatches
                          .map((b) => `#${b.batchNo || b.title}`)
                          .join(", ")
                      : "None"}
                  </span>
                </div>
                <div
                  className="d-flex justify-content-between pt-2 border-top border-secondary"
                  style={{ fontSize: "0.9rem" }}
                >
                  <span className="fw-bold text-white">
                    Total Combined Candidates:
                  </span>
                  <span className="fw-bold text-warning">
                    {consolidateCombinedTotal} students
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="modal-footer px-4 pb-4 pt-2">
              ...
              >
                <GitMerge size={16} className="me-1" style={ICON_STYLE} />{" "}
                {savingConsolidate
                  ? "Consolidating..."
