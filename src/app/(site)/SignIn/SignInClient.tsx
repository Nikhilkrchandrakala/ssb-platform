...
function SignInForm({ initialDisplaySettings }: { initialDisplaySettings: AuthDisplaySettings | null }) {
  ...
  // Auth Display Settings slideshow/ad logic — seeded from the server so the
  // correct banner/slideshow image is in the first paint instead of flashing
  // in ~1s later once a client-side fetch resolves.
  const [displaySettings, setDisplaySettings] = useState<AuthDisplaySettings | null>(initialDisplaySettings);
...
export default function SignInClient({ initialDisplaySettings }: { initialDisplaySettings: AuthDisplaySettings | null }) {
  return (
    <Suspense fallback={null}>
      <SignInForm initialDisplaySettings={initialDisplaySettings} />
    </Suspense>
  );
}
